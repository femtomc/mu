...
describe("ForumStore", () => {
	test("post persists python-compatible row and emits forum.post", async () => {
		...
		const forumPath = join(dir, ".inshallah", "forum.jsonl");
		...
		const msg = await store.post("issue:inshallah-abc123", "hello", "worker");

		expect(msg).toMatchObject({
			topic: "issue:inshallah-abc123",
			body: "hello",
			author: "worker",
		});
		...
		expect(events[0]).toMatchObject({
			...
			payload: { message: { topic: "issue:inshallah-abc123", body: "hello" } },
		});
	});
	...
	test("topics summarizes and sorts by most-recent activity and supports prefix filtering", async () => {
		...
		await writeJsonl(forumPath, [
			...
			{ topic: "issue:inshallah-xyz999", body: "i1", author: "worker", created_at: 4 },
		]);

		const all = await store.topics();
		expect(all.map((t) => t.topic)).toEqual(["issue:inshallah-xyz999", "beta", "alpha"]);
		expect(all.find((t) => t.topic === "alpha")).toMatchObject({ messages: 2, last_at: 2 });

		const issues = await store.topics("issue:");
		expect(issues.map((t) => t.topic)).toEqual(["issue:inshallah-xyz999"]);
	...
	test("schema compatibility: preserves extra keys from legacy rows when posting", async () => {
		...
		await store.post("issue:inshallah-keep", "next", "worker");
