	...
	if (hasHelpFlag(argv0)) {
		return ok(
			[
				...
				"",
				"Session targeting:",
				"  - If --session-kind is omitted, mu turn auto-resolves --session-id across",
				"    both terminal operator and control-plane operator session stores.",
				"  - If the same id exists in both stores, pass --session-kind or --session-dir.",
				"  - For deterministic targeting, --session-kind accepts operator|cp_operator.",
				"",
				"Notes:",
				...
				"",
				"Examples:",
				"  mu turn --session-kind operator --session-id <id> --body \"Summarize current blockers\"",
