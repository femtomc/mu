	...
	if (hasHelpFlag(argv0)) {
		return ok(
			[
				...
				"",
				"Session targeting:",
				"  - If --session-kind is omitted, mu turn defaults to control-plane operator sessions",
				"    (<store>/control-plane/operator-sessions; session_kind=cp_operator).",
				"  - For terminal/tmux sessions from mu serve / mu session, use --session-kind operator",
				"    (<store>/operator/sessions).",
				"",
				"Notes:",
