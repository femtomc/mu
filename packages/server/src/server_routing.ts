...
import { ackSessionFlashRecord, createSessionFlashRecord, sessionFlashRoutes } from "./api/session_flash.js";
import {
	executeSessionTurn,
	parseSessionTurnRequest,
	SessionTurnError,
	sessionTurnRoutes,
} from "./api/session_turn.js";
			...
			switch (kind) {
				...
				case "issue_update": {
					...
					if (Object.keys(patch).length === 0) {
						return Response.json(
							{ error: "issue_update requires at least one patch field" },
							{ status: 400, headers },
						);
					}
				...
				case "issue_dep": {
					...
					if (!srcId || !dstId) {
						return Response.json(
							{ error: "src_id and dst_id are required for issue_dep" },
							{ status: 400, headers },
						);
					}
				...
				case "issue_undep": {
					...
					if (!srcId || !dstId) {
						return Response.json(
							{ error: "src_id and dst_id are required for issue_undep" },
							{ status: 400, headers },
						);
					}
					if (depType !== "blocks" && depType !== "parent") {
				...
				case "session_flash_create": {
					const sessionId = readTrimmedString(body.session_id);
					const messageBody = readTrimmedString(body.body);
					if (!sessionId) {
						return Response.json(
							{ error: "session_id is required for session_flash_create" },
							{ status: 400, headers },
						);
					}
					if (!messageBody) {
						return Response.json(
							{ error: "body is required for session_flash_create" },
							{ status: 400, headers },
						);
					}
					...
					const flash = await createSessionFlashRecord({
						...
						sessionId,
						body: messageBody,
						sessionKind: readTrimmedString(body.session_kind) || null,
						contextIds: readCommaList(body.context_ids),
						source: readTrimmedString(body.source) || "command_api",
						metadata:
							body.metadata && typeof body.metadata === "object" && !Array.isArray(body.metadata)
								? (body.metadata as Record<string, unknown>)
								: {},
						from: {
							channel: readTrimmedString(body.from_channel) || null,
							channel_tenant_id: readTrimmedString(body.from_channel_tenant_id) || null,
							channel_conversation_id: readTrimmedString(body.from_channel_conversation_id) || null,
							actor_binding_id: readTrimmedString(body.from_actor_binding_id) || null,
						},
					});
					return commandCompletedResponse(headers, "session flash create", { flash });
				}
				case "session_flash_ack": {
					const flashId = readTrimmedString(body.flash_id || body.id);
					if (!flashId) {
						return Response.json(
							{ error: "flash_id (or id) is required for session_flash_ack" },
							{ status: 400, headers },
						);
					}
					const flash = await ackSessionFlashRecord({
						repoRoot: deps.context.repoRoot,
						flashId,
						sessionId: readTrimmedString(body.session_id) || null,
						deliveredBy: readTrimmedString(body.delivered_by) || "command_api",
						note: readTrimmedString(body.note) || null,
					});
					if (!flash) {
						return Response.json({ error: "flash not found" }, { status: 404, headers });
					}
					return commandCompletedResponse(headers, "session flash ack", { flash });
				}
				case "session_turn": {
					const parsedTurnRequest = parseSessionTurnRequest(body);
					if (!parsedTurnRequest.request) {
						return Response.json(
							{ error: parsedTurnRequest.error ?? "invalid session_turn request" },
							{ status: 400, headers },
						);
					}
					const requestPayload = {
						...parsedTurnRequest.request,
						source: parsedTurnRequest.request.source ?? "command_api",
					};
					try {
						...
						return commandCompletedResponse(headers, "session turn", { turn });
					} catch (error) {
				...
				case "heartbeat_update": {
					...
					if (!programId) {
						return Response.json(
							{ error: "program_id is required for heartbeat_update" },
							{ status: 400, headers },
						);
					}
				...
				case "heartbeat_delete": {
					...
					if (!programId) {
						return Response.json(
							{ error: "program_id is required for heartbeat_delete" },
							{ status: 400, headers },
						);
					}
				...
				case "heartbeat_trigger": {
					...
					if (!programId) {
						return Response.json(
							{ error: "program_id is required for heartbeat_trigger" },
							{ status: 400, headers },
						);
					}
					if (body.reason != null && typeof body.reason !== "string") {
