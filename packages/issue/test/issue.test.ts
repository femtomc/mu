...
describe("IssueStore", () => {
	test("create persists python-compatible row and emits issue.create", async () => {
		...
		const issuesPath = join(dir, ".inshallah", "issues.jsonl");

		const eventLog = fsEventLog(join(dir, ".inshallah", "events.jsonl"));
		...
		const events = (await readJsonl(join(dir, ".inshallah", "events.jsonl"))) as any[];
	...
	test("claim matches python behavior (ok/failed) and emits issue.claim", async () => {
		...
		expect(await store.claim("inshallah-missing")).toBe(false);
	...
	test("children/subtree_ids/ready/collapsible/validate behave like python", async () => {
		...
		await writeJsonl(issuesPath, [
			...
			},
			{
				...
				outcome: null,
				tags: ["node:agent"],
				deps: [{ type: "parent", target: "inshallah-root" }],
				execution_spec: null,
				priority: 2,
				created_at: 2,
				updated_at: 2,
			...
			{
				id: "inshallah-blocker",
				title: "blocker",
				body: "",
				status: "open",
				outcome: null,
				tags: [],
				deps: [{ type: "blocks", target: "inshallah-leaf2" }],
				execution_spec: null,
				priority: 3,
				created_at: 4,
				updated_at: 4,
		...
		const kids = await store.children("inshallah-root");
		expect(kids.map((k) => k.id).sort()).toEqual(["inshallah-leaf1", "inshallah-leaf2"].sort());

		const subtree = await store.subtree_ids("inshallah-root");
		expect(subtree).toContain("inshallah-root");
		expect(subtree).toContain("inshallah-leaf1");
		expect(subtree).toContain("inshallah-leaf2");

		const readyAll = await store.ready("inshallah-root");
		expect(readyAll.map((i) => i.id)).toEqual(["inshallah-leaf1"]); // leaf2 blocked

		const readyTags = await store.ready("inshallah-root", { tags: ["node:agent", "x"] });
		...
		await store.close("inshallah-blocker", "success");
		const readyAfter = await store.ready("inshallah-root");
		expect(readyAfter.map((i) => i.id)).toEqual(["inshallah-leaf2", "inshallah-leaf1"]);

		const collapsible = await store.collapsible("inshallah-root");
		...
		await store.close("inshallah-leaf1", "success");
		await store.close("inshallah-leaf2", "skipped");

		const collapsible2 = await store.collapsible("inshallah-root");
		expect(collapsible2.map((i) => i.id)).toContain("inshallah-root");

		const v = await store.validate("inshallah-root");
	...
	test("preserves extra fields when round-tripping", async () => {
		...
		await store.update("inshallah-extra", { body: "changed" });
		const rows = (await readJsonl(issuesPath)) as any[];
		expect(rows[0]).toMatchObject({ id: "inshallah-extra", body: "changed", extra_field_ok: true });
	});
});
